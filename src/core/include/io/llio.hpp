...
public:
     ...
     * @return
     */
    Task<size_t> async_write(Path file, const IOBuffer& buffer, bool append = false, EIOPriority priority = EIOPriority::Normal)
    {
        ASSERT(io_backend_);
        return io_backend_->async_write(std::move(file), IOBuffer(buffer), append, priority);
    }
