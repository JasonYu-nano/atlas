...
#ifndef STRING_NAME_CASE_SENSITIVE
#define STRING_NAME_CASE_SENSITIVE 0
#endif
...
public:
    ...
    NameEntryID(uint32 compress_id, uint32 display_id) : compress_id_(compress_id), display_id_(display_id) {};
#else
    NameEntryID(uint32 compress_id) : compress_id_(compress_id) {};
#endif
...
private:
    ...
    uint32 CalcCompressID(ViewType view) const
    {
        if (view.length() <= MAX_ENTRY_LENGTH)
        {
            ...
            {
                lower_case[i] = std::tolower(ViewType::traits_type::to_int_type(view[i]));
            }
