...
public class DefinitionsGenerator(BuildTargetAssembly buildTargetAssembly)
{
    public async Task Generate()
    {
        ...
        {
            var content = GetNoExportDefinitionFileContent(buildTargetAssembly.NameToBuildTargets.Values.ToArray());
    ...
    private string GetDefinitionFileContent(BuildTargetBase buildTarget)
    {
        ...
        stringBuilder.AppendLine();
        stringBuilder.AppendLine($"#undef {buildTarget.ExportStatement}");

        if (buildTarget.NeedExportSymbol())
        {
            stringBuilder.AppendLine();
            stringBuilder.AppendLine($"#define {buildTarget.ExportStatement} DLL_EXPORT"); 
        }
        else
        {
            stringBuilder.AppendLine();
            stringBuilder.AppendLine($"#define {buildTarget.ExportStatement} ");
        }
        ...
        {
            stringBuilder.AppendLine($"#define {linkTarget.ExportStatement} DLL_IMPORT");
        }
    ...
    private string GetNoExportDefinitionFileContent(BuildTargetBase[] buildTargets)
    {
        ...
        {
            stringBuilder.AppendLine($"#define {target.ExportStatement}");
        }
        return stringBuilder.ToString();
    }
}