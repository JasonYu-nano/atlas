...
namespace CppAst
{
    ...
    internal unsafe class CppModelBuilder
    {
        ...
        private bool HasUserAnnotate(CXCursor cursor)
        {
            var hasUserAnnotate = false;
            cursor.VisitChildren((argCursor, parentCursor, clientData) =>
            {
                if (argCursor.Kind == CXCursorKind.CXCursor_AnnotateAttr)
                {
                    hasUserAnnotate = true;
